...
#### 3.2.1 Actors
The player is represented by the `Player` struct.
Pikachu is represented by the `Pikachu` struct.
Obstacles are represented by the `Obstacle` struct.
A Pokeball is represented by the `Pokeball` struct.
##### 3.2.2.3 Bonuses
A bonus is represented by the `Bonus` struct. There are 3 bonus types (enum `BONUS_TYPE`):
...
The first step is to check player input using the `check_game_input` method.
#### 3.3.2 Status Update
...
### 3.5 Highscores
Highscores are maintained in `highscores.h`:
